...
void zcpu_step(void) {
    ...
    if (pc == 0x000100A0u || pc == 0x000100A8u || pc == 0x000100C0u) {
        static uint32_t hits = 0;
        if (hits < 12) {
            LOGI("watch factory 0x%05X: r0=0x%08X r1=0x%08X r4=0x%08X r5=0x%08X r6=0x%08X",
                 pc, g_cpu.r[0], g_cpu.r[1], g_cpu.r[4], g_cpu.r[5],
                 g_cpu.r[6]);
            hits++;
        }
    }
