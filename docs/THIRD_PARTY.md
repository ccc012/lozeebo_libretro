...
| --- | --- | --- |
| Zeemu, local reference copy (`reference/zeemu-main/` in the repo, gitignored and never committed; also kept outside the repo at `C:\Users\Lucas\source\repos\zeemu_reference`) | GPL-3.0 | Used as a reference only for BREW `IShell` behavior, service/applet CLSIDs, timer/resource handling expectations, and compatibility stubs needed by real Zeebo games such as Pac-Mania. Kept locally (not committed) to avoid bloating repo size; behavior is reimplemented independently, not copied. |
...
| GGZ BREW Tools (`reference/ggzbrewtools-main/`, Tuxality, 2021) | Permissive zlib-style license | Compatible with this GPLv3 project as long as its copyright and license notice remain in adapted source files. Documents the big-endian GGZ entry table and concatenated GZIP member layout used by some BREW/Zeebo games. |
...
| OpenZeebo (`reference/openzeebo-master/`) | See upstream repo | Real-hardware reverse-engineering tools (JTAG, NAND, bootloader upload). Kept locally as reference for hardware-accurate behavior; not currently wired into the core. Gitignored, not committed. |
| zeebo_doom (`reference/zeebo_doom-main/`) | See upstream repo | A homebrew Doom port for Zeebo; kept as an example of another BREW-based app running on the real console. Gitignored, not committed. |
