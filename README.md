...
[![Build](https://img.shields.io/badge/build%20validado-Windows%20%7C%20VS2022%20%7C%20x64-lightgrey)]()
...
- **Loader**: `.mod` (variante com header `BREW` e variante binário ARM cru) e resolução de CLSID
  ...
  apenas detecção/existência (parsing ainda pendente).
...
### Windows (Visual Studio 2022) — caminho validado
...
Resultado: `x64\Release\zeebo_libretro.dll` (também há `Debug`/`Win32` combináveis, ver
`.sln`/`.vcxproj`).
...
### Linux/macOS (Makefile) — atualmente desatualizado, não linka

O `Makefile` na raiz existe e detecta a plataforma automaticamente (`zeebo_libretro.so` no Linux,
`.dylib` no macOS), mas sua lista de fontes (`SOURCES`) ainda está presa na fase de esqueleto do
projeto: só compila `src/core/libretro_core.c`, com os grupos de CPU/memória comentados e nenhuma
menção a `src/gpu`, `src/audio`, `src/input`, `src/brew` ou `src/loader` — que `libretro_core.c`
já usa. Rodar `make` hoje falha no link por símbolos indefinidos. Além disso, o script de versão
`src/core/link.T` referenciado pela regra de link do Linux não existe no repositório. **O
caminho validado e mantido de fato é o do Visual Studio**; o Makefile precisa ser atualizado para
refletir o conjunto atual de fontes antes de ser considerado funcional. Alvos disponíveis, uma vez
corrigido: `make`, `make clean`, `make fclean`, `make rebuild`, `make install`, `make info`,
`make help`.

### CMake — apenas placeholder

`CMakeLists.txt` hoje é literalmente um comentário de uma linha (`# CMake build system`) — sem
`project()`, sem alvos. Não é um caminho de build funcional ainda, apenas um lugar reservado para
o futuro.

### Dependências externas

Nenhuma, além de um compilador C e (no Windows) o Windows SDK que vem com o Visual Studio. O core
é C autocontido — nem `zlib`, `libpng`, SDL ou similares são usados dentro de `src/`.
...
`tests/libretro_smoke.c` é um host libretro mínimo (Windows/MSVC) que carrega a DLL diretamente
via `LoadLibraryA` e roda 180 frames, sem depender da interface do RetroArch — o método preferido
...
- [ ] Atualizar o `Makefile` para refletir o conjunto atual de fontes (Linux/macOS não linkam
      hoje — ver seção de build acima).
