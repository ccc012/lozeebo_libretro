...
//
/*
 static int I_TranslateKey(int id)
 {
   int rc = 0;

   switch (id) {
   case ZEEBO_BUTTON_HOME: rc = KEYD_LEFTARROW;  break;
...
void I_FinishUpdate (void)
{
  ...
  // SDL_Flip(screen);
  // printf("Before BREW_Blit");
  // printf("screens[0].data = %x", screens[0].data);
  // printf("screens[0].width = %u", screens[0].width);
  // printf("screens[0].height = %u", screens[0].height);
  // printf("screens[0].depth = %u", V_GetModePixelDepth(V_GetMode()));
  BREW_Blit(screens[0].data, screens[0].width, screens[0].height, V_GetModePixelDepth(V_GetMode()));
  //printf("after Brew blit");
}
...
void I_CalculateRes(unsigned int width, unsigned int height)
{
  ...
  SCREENWIDTH = width;
  SCREENHEIGHT = height;
  SCREENPITCH = (SCREENWIDTH + (512 - SCREENWIDTH)) * V_GetPixelDepth() ;
